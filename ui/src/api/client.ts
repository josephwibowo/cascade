...
export type PendingException = {
  id: string; campaign_id: string; account_id: string; account_name?: string | null; arr?: number | null;
  account_status?: string | null; risk?: string | null; blocker_type?: string | null; exception_type: string;
  hitl_task_id?: string | null; airflow_dag_run_id?: string | null; status: string;
...
export const api = {
  ...
  accounts: (id: string, filters: Record<string, string>, limit = 200, offset = 0) => {
    const params = new URLSearchParams({...filters, limit: String(limit), offset: String(offset)});
    return request<{items: AccountRow[]; total: number; limit: number; offset: number; facets: {chips: Record<string, number>}}>(`/cascade/campaigns/${encodeURIComponent(id)}/accounts?${params}`);
  },
  ...
  advance: (snapshot: string) => request<{snapshot: string; dag_run_id?: string; run_id?: string}>('/cascade/scenario/advance', {method: 'POST', body: JSON.stringify({snapshot})}),
  scenario: () => request<{snapshot: string}>('/cascade/scenario'),
  verify: (id: string) => request<{dag_run_id?: string; run_id?: string}>(`/cascade/campaigns/${encodeURIComponent(id)}/verify`, {method: 'POST'}),
};
